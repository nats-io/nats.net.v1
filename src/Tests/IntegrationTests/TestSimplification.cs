...
namespace IntegrationTests
{
    ...
    {
        public TestSimplification(SimplificationSuiteContext context) : base(context) {}
        [Fact]
        public void testStreamContext() {
            Context.RunInJsServer(c => {
                ...
                IStreamContext streamContext = js.CreateStreamContext(STREAM);
        ...
        public void testConsumeManual() {
            Context.RunInJsServer(c => {
                ...
                // create the consumer then use it
                IManualConsumer consumer = consumerContext.consume();
        ...
        public void testConsumeWithHandler() {
            Context.RunInJsServer(c => {
                ...
                InterlockedInt atomicCount = new InterlockedInt();
                EventHandler<MsgHandlerEventArgs> handler = (s, e) => {
                    e.Message.Ack();
                    if (atomicCount.Increment() == stopCount)
                    {
                        latch.Signal();
                    }
                };
    
                ISimpleConsumer consumer = consumerContext.consume(handler);
                latch.Wait();
                consumer.Stop(200);
                Assert.True(atomicCount.Read() > 500);
            });
        }
        ...
        private void closeConsumer(ISimpleConsumer con, string name, bool doStop) {
        ...
        public void testConsumeOptionsBuilder() {
            ...
            co = ConsumeOptions.Builder().WithBatchBytes(1000).WithThresholdPercent(50).Build();
            Assert.Equal(DefaultMessageCountWhenBytes, co.BatchSize);
            Assert.Equal(1000, co.BatchBytes);
            Assert.Equal(50, co.ThresholdPercent);
    
            Assert.Throws<ArgumentException>(() => ConsumeOptions.Builder().WithThresholdPercent(0).Build());
            Assert.Throws<ArgumentException>(() => ConsumeOptions.Builder().WithThresholdPercent(-99).Build());
            Assert.Throws<ArgumentException>(() => ConsumeOptions.Builder().WithThresholdPercent(101).Build());
            Assert.Throws<ArgumentException>(() => ConsumeOptions.Builder().WithExpiresIn(0).Build());
