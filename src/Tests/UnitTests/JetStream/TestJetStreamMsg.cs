...
namespace UnitTests.JetStream
{
    public class TestJetStreamMsg : TestBase
    {
        ...
        const string InvalidMetaLt8Tokens = "$JS.ACK.less-than.9-tokens.1.2.3";
        ...
        public void MetaDataTests()
        {
            ...
            Assert.Throws<NATSException>(() => new MetaData(InvalidMetaNoAck));
            Assert.Throws<NATSException>(() => new MetaData(InvalidMetaLt8Tokens));
            Assert.Throws<NATSException>(() => new MetaData(InvalidMeta10Tokens));
            Assert.Throws<NATSException>(() => new MetaData(InvalidMetaData));
        }
